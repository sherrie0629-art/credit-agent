...
export async function streamImage(
  ...
  if (!res.ok || !res.body) {
    ...
    if (res.status === 429) throw new Error("AI 网关限流，请稍后重试");
    if (res.status === 402) throw new Error("AI 额度已用尽，请在工作区补充额度");
  ...
  const parser = createParser({
    onEvent(event) {
      ...
      if (event.event === "error" || payload?.type === "error") {
        streamError =
          (payload as { error?: { message?: string } })?.error?.message ?? "图像生成失败";
        return;
      }
      if (
  ...
  if (!sawCompleted) throw new Error("图像流意外结束");
}
